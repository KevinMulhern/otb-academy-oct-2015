RSpec.describe "Class methods" do
  ...
  it "all classes are Classes" do
    expect( AnotherDog.is_a?(Class) ).to eq( true )
  end

  it "classes are objects too" do
    expect( AnotherDog.is_a?(Object) ).to eq( true )
  end
  ...
  it "has methods on classes" do
    expect( AnotherDog.methods.size > 0 ).to eq( true )
  end
  ...
  class AnotherDog 
    def AnotherDog.a_class_method
      :dogs_class_method
    end
  end

  it "is possible to define a class method inside the class" do
    expect( AnotherDog.a_class_method ).to eq( :dogs_class_method )
  end

  LastExpressionInClassStatement = class AnotherDog
                                     21
                                   end
  ...
  SelfInsideOfClassStatement = class AnotherDog
                                 self
                               end

  it "uses self to refer to the class, not an instance inside the class statement" do
    expect( AnotherDog == SelfInsideOfClassStatement ).to eq( true )
  end
  ...
  it "is possible to use self to define a class method" do
    expect( AnotherDog.class_method2 ).to eq( :another_way_to_write_class_methods )
  end
  ...
  it "has a third way to define a class method" do
    expect( AnotherDog.another_class_method ).to eq( :still_another_way )
  end
